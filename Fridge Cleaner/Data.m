...
-(void) reload
{
    [self loadDatabase];
    if (selectedRecipe)
        [self loadFullRecipeWithKey:[selectedRecipe intValue]];
    
    NSArray* array = [selectedIngridients retain];
    self.selectedIngridients = nil;
    self.selectedIngridients = [[NSMutableArray alloc] init];
    for (NSNumber* num in array)
    {
        Ingridient* ing = [ingridients objectAtIndex:[num intValue]-1];
        [self addIngridientToSelected:ing];
    }
    [array release];
    
    [self updateLoadedRecipes];
}
...
-(void) releaseData
{
    if (selectedRecipe)
    {
        RecipeFull* rec = selectedRecipe;
        self.selectedRecipe = [NSNumber numberWithInt:rec.key];
    }
    ...
    self.ingridients = [[NSArray alloc] init];
    self.ingridientsLocalized = [[NSMutableArray alloc] init];
    self.recipes = [[NSArray alloc] init];
    self.loadedRecipesArray = [[NSMutableArray alloc] init];
    self.loadedRecipesSections = [[NSMutableArray alloc] init];
}

-(void) loadFullRecipeWithIndexPath:(NSIndexPath*)path
{
    NSArray* array = [loadedRecipesArray objectAtIndex:[path section]];
    Recipe* rec = [array objectAtIndex:[path row]];
    [self loadFullRecipeWithKey:rec.key];
}

-(void) loadFullRecipeWithKey:(int)key
{
    NSString* opath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"data.sqlite"];
    sqlite3* database;
    
    if (sqlite3_open([opath UTF8String],&database) == SQLITE_OK)
    {
        self.selectedRecipe = [[RecipeFull alloc] initWithPrimaryKey:key database:database];
        sqlite3_close(database);
    }
}
